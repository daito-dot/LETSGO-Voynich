#!/usr/bin/env python3
"""Issue #75 Phase E target-blind M5 candidate-owned-null smoke."""
...
import phase75e_score as score  # noqa: E402


def _forbidden_target_loader(*_args, **_kwargs):
    raise RuntimeError("TARGET FIREWALL: target loader called during Phase-E candidate smoke")
...
def main(argv: Sequence[str]) -> int:
    ...
    authority, cases, e0_sha = score.load_e0()
    candidate, X, audit = score.build_exact_case(src, rep, authority, cases, e0_sha)
    ...
    if len(m["z_full"]) != 66 or int(m["n_reference"]) != 1000 or int(m["n_test"]) != 1000:
        raise RuntimeError("candidate-owned null contract changed")

    result = {
        "schema": "issue75-phaseE-m5-candidate-null-smoke-v1",
        "status": "M5_CANDIDATE_OWNED_NULL_EXECUTION_VERIFIED_TARGET_BLIND",
