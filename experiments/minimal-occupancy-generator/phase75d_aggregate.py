...
def load_controls() -> tuple[dict, dict, list[float], list[float]]:
    ...
    if c.get("schema") != "issue75-phaseC-m3-aggregate-v1":
        raise RuntimeError("Phase C aggregate schema changed")
    if c.get("population", {}).get("total_results") != N_REPS or c.get("population", {}).get("complete_population") is not True:
        raise RuntimeError("Phase C population authority changed")
    if c.get("primary", {}).get("classification") != "M3_KRS_NEAREST_NEIGHBOR_TRANSITION_GRAMMAR_INSUFFICIENT_NONLOCAL_OR_LATENT_RULE_REQUIRED":
        raise RuntimeError("Phase C classification authority changed")
    if c.get("primary", {}).get("no_material_loss") is not False:
        raise RuntimeError("Phase C sufficiency authority changed")
    ...
    if abs(float(statistics.median(t3)) - float(c["measurement"]["T"]["median"])) > 1e-12:
        raise RuntimeError("Phase C paired T reconstruction mismatch")
    return a, c, plus, t3


def validate_case(r: Mapping, rep: int, d0_sha: str, frozen_target_authority: Mapping) -> None:
    ...
    for key in (
        "training_unary_moments_exact_d0",
        "training_adjacent_moments_exact_d0",
        "training_distance_moments_exact_d0",
        "descriptor_distribution_exact_d0",
        "rank_29_revalidated",
        "fit_tolerance_revalidated",
    ):
    ...
    if r.get("target_authority") != frozen_target_authority:
        raise RuntimeError(f"target authority differs from frozen Phase C authority rep {rep}")
...
def main(argv: Sequence[str]) -> int:
    ...
    frozen_target_authority = c["target_authority"]
    ...
    for p in paths:
        ...
        validate_case(r, rep, d0_sha, frozen_target_authority)
        rows[rep] = r
    if set(rows) != set(range(N_REPS)):
        raise RuntimeError("complete Phase D population mismatch")
    ...
    result = {
        ...
        "target_authority": frozen_target_authority,
        "measurement": {
            "T": summary(T),
        ...
        "primary": {
            "D_M4_minus_phaseA_plus_center": d4,
            "gap_M4": gap,
        ...
        "secondary_nonpromoting": {
            "T_M3_reconstructed_from_frozen_phaseC_paired_values": t3,
            "T_M4_minus_M3": versus_m3,
            "summary_T_M4_minus_M3": summary(versus_m3),
            "phase_c_classification": c["primary"]["classification"],
        ...
        "model_complexity": {
            "free_continuous_parameters_per_fold": 29,
            "free_unary_parameters": 11,
            "free_adjacent_parameters": 10,
            "free_generic_nonadjacent_distance_parameters": 8,
            "explicit_named_nonadjacent_pair_parameters": 0,
        ...
        "guardrails": {
            ...
            "latent_architecture_selected": False,
    ...
    print(json.dumps({
        ...
        "gap_M4": gap,
        "median_T_M4_minus_M3": result["secondary_nonpromoting"]["summary_T_M4_minus_M3"]["median"],
        "delta_plus_q95": DELTA,
