#!/usr/bin/env python3
"""Phase63B parser-only preflight; computes no edit-distance/scientific metric."""
...
def summary(path: Path, label: str, view: str) -> dict:
    ...
    types = set(tokens)
    base = [it for it in its if q.b.base_eligible(it)]
    s1 = [it for it in base if q.b.valid_pseudo_indices(it)]
    ...
    return {
        **audit,
        ...
        "base_eligible_paragraphs": len(base),
        "S1_eligible_paragraphs": len(s1),
