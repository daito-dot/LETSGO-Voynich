...
# Dynamic exact path multiplicity per literal surface. Start with all-absent path.
counts = {"": 1}
for vals in SLOTS:
    nxt = {}
...
# Deterministic S0-16 codebook = first 65536 lexicographic distinct strings.
if u_count >= 65536:
...
for i, s in enumerate(codebook):
    if parser.pick(s, "min") is not None:
        accepted += 1
    if inverse.get(s) == i:
        roundtrip += 1
...
result = {
  ...
  "parse_multiplicity_histogram": {str(k): v for k,v in sorted(parse_mult_hist.items())},
  "floor_log2_distinct_surfaces": k,
  "distinct_surface_length_distribution": {str(k):v for k,v in sorted(length_dist.items())},
...
with open('/tmp/issue184_s0_result.json','w',encoding='utf-8') as f:
