...
| H46-R | Large paragraph-boundary edit1-family discontinuity is unusual relative to current prose/cipher controls | REFINED BY PHASE59-60 | Medieval source-native item entries reproduce part of the transition; a Voynich-specific specialization remains. |
...
| H59B-2 | Voynich entry transition is absent from all medical controls | REJECTED AS STATED | Source-native medical entries reproduce part of the transition. |
| H59B-3 | Mixed structured medieval genres reproduce part of the entry phenomenon | SUPPORTED | Medical and nonmedical source-marker controls show substantial entry transitions. |
| H59C-1 | Generic medieval entry grammar is sufficient for Voynich | NOT SUPPORTED | External rank-2 entry basis explains substantial but incomplete transition energy; orthogonal remainder persists. |
| H59C-2 | Voynich combines shared medieval entry grammar with a Voynich-specific component | SUPPORTED STRUCTURE | External component plus stable orthogonal remainder; remainder positive across H/B/P/S/T. |
| H59C-3 | External entry similarity is fully stable to control composition | PARTIAL WARNING | Leave-one-control-class-out basis varies because external sample is small/heterogeneous, though remainder does not collapse. |
